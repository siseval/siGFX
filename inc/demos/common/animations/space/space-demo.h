...
public:
    ...
    std::shared_ptr<Body> spawn_body(const gfx::Vec2d position, const gfx::Vec2d velocity, const double radius, const double mass, const gfx::Color4 color);
    std::shared_ptr<Body> spawn_body(const std::string name, const gfx::Vec2d position, const gfx::Vec2d velocity, const double radius, const double mass, const gfx::Color4 color);
    std::shared_ptr<Body> spawn_body(const std::string name, const gfx::Vec2d position, const gfx::Vec2d velocity, const double radius, const double mass, const bool locked, const gfx::Color4 color);
    ...
    void set_camera_pos(const gfx::Vec2d pos) { camera.cur_pos = pos; }
    void set_camera_size(const double size) { camera.size_cur = gfx::Vec2d { size, size * get_resolution().y / get_resolution().x }; }

    gfx::Vec2d get_screen_pos(const gfx::Vec2d world_pos);
    gfx::Vec2d get_world_pos(const gfx::Vec2d screen_pos);

    inline gfx::Box2d get_view_bounds() const { return view_bounds; }

    inline gfx::Vec2d get_anchor_world_pos() { return view_bounds.min + view_bounds.size() * view_anchor; }
    inline gfx::Vec2d get_anchor_screen_pos() { return view_bounds.size() * view_anchor; }

    std::shared_ptr<Body> get_closest_body(const gfx::Vec2d position, const double max_distance = 0.0);
    std::shared_ptr<Body> get_closest_body_screenspace(const gfx::Vec2d screen_pos, const double max_distance = 0.0, const double equality_threshold = 2.0, const double time_lerp = 1.0);
...
private:
    ...
    void progress_spawn_workflow(const gfx::Vec2d position);
    ...
    gfx::Vec2d body_interpolated_pos(const std::shared_ptr<Body> body, const double time_lerp = 1.0);

    void set_view_pos(const gfx::Vec2d pos);
