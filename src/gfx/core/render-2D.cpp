...
namespace gfx
{
    ...
    std::shared_ptr<Polyline2D> Render2D::create_polyline(
        const Vec2d position,
    ...
    std::shared_ptr<Polygon2D> Render2D::create_polygon(
        const Vec2d position,
        const std::vector<Vec2d> &points,
        const Color4 color
    )
    ...
    std::shared_ptr<Text2D> Render2D::create_text(
        const Vec2d position,
